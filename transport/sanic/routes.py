...
def get_routes(config: ApplicationConfig, context: Context) -> Tuple:
    return (
        ...
        endpoints.EmployeeEndpoint(config, context, uri='/employee/<eid:int>', methods=['PATCH', 'DELETE'],
                                   auth_required=True),
        endpoints.AllEmployeeEndpoint(config, context, uri='/employee/all', methods=['GET'], auth_required=True)
    )
